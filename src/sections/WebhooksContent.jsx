...
const WebhooksContent = ({ onLaunchGuide }) => (
  <article className="flex-1 min-w-0 animate-in fade-in duration-300 pb-20">
    <div className="mb-10">
      ...
      <h1 className="text-4xl font-bold text-slate-900 tracking-tight mb-6">Webhooks</h1>
      ...
      <p className="text-lg text-slate-600 leading-relaxed">Real-time notifications for signature events so you can update internal systems without polling.</p>
      ...
      <button onClick={onLaunchGuide} className="mt-4 text-blue-600 font-semibold hover:underline flex items-center gap-1">
    ...
    <section id="hooks-events" className="mb-16 scroll-mt-32">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {[{ name: "SIGNATURE_REQUEST_COMPLETED", desc: "All parties have signed the document." }, { name: "SIGNATURE_REQUEST_DECLINED", desc: "A recipient refused to sign." }, { name: "RECIPIENT_SIGNED", desc: "One specific recipient completed their part." }, { name: "RECIPIENT_ADDED", desc: "A new signer was added to the request." }].map((evt, i) => (
            <div key={i} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
              <code className="text-xs font-bold text-purple-700 block mb-1">{evt.name}</code>
              <span className="text-sm text-slate-600">{evt.desc}</span>
            </div>
          ))}
        </div>
        <p className="text-sm text-slate-600 mt-4">Use webhooks to update internal systems, trigger automations, or notify users without polling the API.</p>
    </section>
    ...
    <section id="hooks-security" className="mb-16 scroll-mt-32">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex gap-3 text-sm text-green-900 mb-6">
            <p>Never expose your Webhook Secret in client-side code. Verification must happen on your secure backend server.</p>
        </div>
        <div className="space-y-6">
            <div>
                <h4 className="font-bold text-slate-700 mb-2">Node.js</h4>
                <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs">
                    <CodeBlock language="javascript" code={`const expected = crypto.createHmac('sha256', secret).update(payload).digest('hex');`} />
                </div>
            </div>
            <div>
                <h4 className="font-bold text-slate-700 mb-2">Python</h4>
                <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs">
                    <CodeBlock language="python" code={`expected = hmac.new(secret.encode(), payload, hashlib.sha256).hexdigest()`} />
                </div>
            </div>
        </div>
        <div className="mt-6 bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm text-slate-600">
          <h4 className="font-semibold text-slate-800 mb-2 text-sm">Delivery & retries</h4>
          <ul className="list-disc list-inside space-y-1">
            <li>Expect exponential backoff retries on non-200 responses.</li>
            <li>Log and validate <code className="font-mono text-xs">X-CovoSign-Signature</code> headers.</li>
