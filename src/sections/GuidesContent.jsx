...
const GuidesContent = ({ onLaunchGuide, onViewStatuses, onViewApiKeys }) => (
  <article className="flex-1 min-w-0 animate-in fade-in duration-300 pb-20">
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 mb-8 flex items-center gap-3 text-sm">
      <div className="bg-white p-1.5 rounded-md border border-slate-200 shadow-sm text-slate-500"></div>
    ...
    <div className="mb-10">
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
        <p className="text-sm text-green-800">
          This section has step-by-step guides to help you create your first
          signature request using CovoSign. It's like a tutorial that walks you
          through the process, from setting up your environment to sending the
          document for signing. Even if you're new to coding, you can follow
          along and see how it works in a safe sandbox environment first.
        </p>
      </div>
      <p className="text-lg text-slate-600 leading-relaxed mb-8">
        Step-by-step guide to configure your environment, create, and activate
        your request, mirroring the production flow.
      </p>

      <div
        onClick={onLaunchGuide}
        className="rounded-xl p-8 border border-slate-200 bg-white cursor-pointer transition-colors hover:border-slate-300"
      >
        ...
        <p className="text-slate-600 mb-6 max-w-md">
          Launch the walkthrough to see the Python implementation end-to-end.
        </p>
      ...
      <div className="mt-4 grid md:grid-cols-2 gap-4">
        ...
        <div className="border border-slate-200 rounded-lg p-4 bg-slate-50">
          ...
          <p className="text-sm text-slate-600">
            ...
            and production keys when ready. Workflows stay the same.
          </p>
        </div>
      </div>
    </div>
    ...
    <section id="step-config" className="space-y-6 mb-16 scroll-mt-32">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm font-mono text-slate-600">
        <span className="bg-slate-200 text-slate-700 px-2 py-0.5 rounded text-xs font-bold mr-2">
          FILE
        </span>{" "}
        .env
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        <div>
          ...
          <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs h-full">
            <CodeBlock
              language="json"
              code={`{ "title": "NDA Agreement", "delivery_mode": "email" }`}
            />
          </div>
        </div>
        <div>
  ...
  "data": { 
    ...
    <section id="step-signer" className="space-y-6 mb-16 scroll-mt-32">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm font-mono text-slate-600">
        ...
        /signature_requests/{`{id}`}/recipients
      </div>
      <div className="grid grid-cols-1 gap-4">
        <div>
          <div className="text-xs font-semibold text-slate-500 uppercase mb-2">
          ...
          <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs h-full">
            <CodeBlock
              language="json"
              code={`{ "email": "alice@example.com", "name": "Alice", "role": "SIGNER", "order": 1 }`}
            />
          </div>
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 gap-4">
        <div>
          ...
          <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs h-full">
            <CodeBlock
              language="json"
              code={`{ "recipientId": "650e8400-e29b-41d4-a716-446655440001", "type": "SIGNATURE", "page": 1, "x": 100, "y": 200, "width": 200, "height": 50, "name": "Signature_1" }`}
            />
          </div>
        </div>
        <div>
    ...
    <section id="step-activate" className="space-y-6 pb-20 scroll-mt-32">
      ...
      <div className="grid grid-cols-1 gap-4">
        <div>
          ...
          <div className="bg-[#1e2329] rounded-lg p-5 text-white text-xs h-full flex items-center justify-center text-slate-500 italic">
            (No Request Body)
          </div>
        </div>
        <div>
