...
const CodeBlock = ({ code, language = "python" }) => {
  ...
  /* 
   * Moved formattedCode definition above tokens to avoid ReferenceError
   */
  ...
  const tokens = useMemo(() => {
    const highlightCode = (text) => {
      ...
      const patterns = [
        ...
        { type: "string", regex: /(['"])(?:(?=(\\?))\2.)*?\1/ },
        {
          type: "keyword",
          regex:
            /\b(def|class|return|import|from|if|else|elif|True|False|None|with|as|for|in|try|except|public|private|protected|void|static|new|async|await|const|let|var|function)\b/,
        },
        {
          type: "builtin",
          regex:
            /\b(print|open|len|str|int|list|dict|set|bytes|hmac|hashlib|logging|console|Math|JSON|Promise)\b/,
        },
    ...
    return highlightCode(formattedCode || "");
  }, [formattedCode]);
